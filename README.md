...
```markdown
![傍晚的海](图片地址)                    与正文同宽（默认）
![码头](图片地址   "[left] 码头")         图片靠左，紧随其后的文字绕排在右边
![招牌](图片地址   "[right] 招牌")        图片靠右，紧随其后的文字绕排在左边
![傍晚的海](图片地址 "[wide] 傍晚的海")   出血，比正文栏宽（适合风景）
![傍晚的海](图片地址 "[full] 傍晚的海")   占满整个屏幕宽（一篇别超过两张）
![码头](图片地址   "[half] 码头")        缩到 58% 居中
![招牌](图片地址   "[small] 招牌")        缩到 78% 居中
```

方括号里的指令**不会显示出来**，图注就是后面的文字。手机上所有尺寸会自动还原成满宽，不会挤出屏幕。

`[left]` / `[right]` 的要点：**文字要写在图片后面**，它才会绕排到图片旁边。写在图片前面的话，那段文字会占满整行，图片只能另起一行。
...
|---|---|---|
| 月汀里海滩 | `[full]` | 开场，满屏铺开 |
| 咸德红航标 | `[left]` | 图片在左，长街那段文字绕排在右 |
| 咸德长街 | `[wide]` | 展开，比正文栏宽 |
| 海边演出 | `[right]` | 图片在右，乐队那两段文字绕排在左 |

一放一收，读起来有节奏。

> 技术上有个坑值得记一笔：Markdown 里独占一段的图片会被包进 `<p>`，而 `<p>` 里**不能放块级元素**（`<figure>`、`<div>` 都是非法的，浏览器会强行把段落拆开，排版就乱了）。
> 所以这里用 `<span>` 当容器——span 是行内标签，套在 `<p>` 里合法——再用 CSS 给它 `display:block` / `float`。既能整体控制，又不破坏 HTML。
