...
**所有照片宽度一致**，和正文栏同宽，图片下面是居中的一行小字（图注）。
没有左右浮动、没有尺寸变化——图片和文字永远是上下排列，不并排。

想改的话只有两个地方：

- 图注的字体和颜色 → `static/css/post-figures.css`
- 图片和正文栏的宽度 → 主题的 `--container-width`（在 `themes/blank-magazine/static/css/style.css` 里）

> 一个实现上的细节：Markdown 里独占一段的图片会被包进 `<p>`，而 `<p>` 里**不能放块级元素**
> （`<figure>`、`<div>` 都非法，浏览器会强行拆开段落，排版就散了）。
> 所以渲染钩子用 `<span>` 当容器——span 是行内标签，套在 `<p>` 里合法——再用 CSS 给它 `display:block`。

## 大图查看器

点任意照片打开。关闭按钮在右上角（`Esc` 也行），左右箭头切换，底部信息条依次是
**序号 / 图注 / 拍摄参数**。

相机参数**只在查看器里显示**，正文里保持干净。

实现全在 `static/css/photo-viewer.css` 和 `static/js/photo-viewer.js` 两个文件里，
不依赖主题的任何样式，文章和相册共用。配色和尺寸集中在 CSS 顶部那组 `--pv-*` 变量里。
